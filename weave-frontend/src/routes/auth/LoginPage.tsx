  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-ink px-4 py-16">
      ...
      <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.45, delay: 0.08 }} className="w-full max-w-[420px]">
        <AuthCard>
          <div className="mb-6">
            <span className="font-mono text-[11px] uppercase tracking-wider text-text-muted">Welcome back</span>
            <h1 className="mt-1.5 font-display text-2xl font-semibold tracking-tight text-text-primary">Log in to Weave</h1>
          </div>
          ...
          <div className="space-y-4">
            ...
            <AuthInput
              ...
              placeholder="••••••••"
              hint={
                <a href="/forgot-password" className="font-mono text-[11px] text-text-muted transition-colors hover:text-text-primary">
                  Forgot password?
                </a>
              }
              right={
                <button type="button" onClick={() => setShow(!show)} className="text-text-muted transition-colors hover:text-text-primary">
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 rounded-btn border border-signal-red/30 bg-signal-red/10 px-3 py-2.5">
          ...
          <p className="mt-6 text-center font-mono text-[11px] text-text-muted">
            No account?{' '}
            <Link to="/signup" className="text-accent underline underline-offset-2 transition-opacity hover:opacity-80">
              Sign up free
            </Link>
          </p>
        </AuthCard>
